...
const children = CONTAINER_CHILDREN["#rightcontainer"].forEach(child => {
    if (!document.querySelector(child)) childrenLoaded = false;
})
...
console.log(childrenLoaded)
    ...
    function enableBlockReordering(containerSelector) {
        ...
        const children = CONTAINER_CHILDREN[containerSelector].forEach(child => {q()})

        const container = q(containerSelector);
        function wait() {
            if (!container) {
                console.log("Waiting for container or children to load for block reordering...");
                container = q(containerSelector);
                setTimeout(wait, 100);
            } else {
                console.log("Container loaded, enabling block reordering...")
                // apply stored order
    ...
    function setupGlobalEdit() {
        ...
        function wait() {
            if (!menu) {
                console.log("Waiting for menu to load for global edit setup...");
                menu = q(".topnav__menu");
                setTimeout(wait, 100);
            } else {
                console.log("Menu loaded, setting up global edit...")
    ...
    function fetchBarcode() {
        ...
        if (cached) {
            console.log("Using cached barcode data");
            const barcodeDivsStyle = JSON.parse(cached);
            initializeBarcode(barcodeDivsStyle);
            return;
        }

        console.log("Fetching barcode data from server...");
        GM_xmlhttpRequest({
            method: "GET",
    ...
    function initializeBarcode(barcode_list) {
        ...
        function wait() {
            if (!menu) {
                ...
                console.log("Menu loaded, initializing barcode button...");
