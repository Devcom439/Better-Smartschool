...
(function () {
    ...
    function simulateClickWhenReady(el, times = 1, interval = 100, timeout = 5000) {
    ...
    function parseSchedule(html) {
        ...
        for (const row of qa("tr", doc)) {
            ...
            const classGroups = cols[1]?.innerText.trim().split(",").reverse() || "";
            let forSpecificGroup = false
            let specificGroup = null

            for (let classGroup of classGroups) {
                if (classGroup.toLowerCase().includes("klasdeel")) {
                    specificGroup = "(".concat(classGroup.split("(")[1])
                ...
                if (forSpecificGroup && specificGroup) {
                    classGroup = classGroup.concat(" ", specificGroup)
                    forSpecificGroup = false
                }
    ...
    function insertScheduleIntoPage(schedule) {
        ...
        for (let i = 0; i < schedule.length; i++) {
            ...
            html.push(`
                <p style="margin:0;padding:2px 0;">👥Wie: ${item.classGroup}</p>
            ...
            if (i < schedule.length - 1 && next && next.day === item.day) {
    ...
    function parseAndInsertPoints(html) {
        ...
        const totalPercentage = totalPossible > 0 ? `(${((totalObtained / totalPossible) * 100).toFixed(2)}%)` : "";

        // Save updated seenTests
        localStorage.setItem("seenTests", JSON.stringify(seenTests));

        insertPointsIntoPage(result, newTests, totalPercentage);
    }

    function insertPointsIntoPage(subjects, newTests, totalPercentage) {
        const container = q("#rightcontainer");
        if (!container) return;
      ...
      <div class="homepage__block__top">
        <div class="homepage__block__top__title">
          <h2 class="smsc-title--1" style="color:#000">Punten ${totalPercentage}</h2>
        </div>
        <div class="homepage__block__top__buttonbar"></div>
      </div>
      <div class="homepage__block__content">${html.join("")}</div>
    ...
    function handleKeydown(event) {
        ...
        if (!isInput && !window.__gEventActive && !window.__lEventActive) {
            switch (event.key.toLowerCase()) {
                case "s":
                    ...
                    simulateClickWhenReady(q(".js-btn-home"));
                    return;
                case "b":
                    event.preventDefault();
                ...
                case "v":
                    event.preventDefault();
                    simulateClickWhenReady(q(".js-btn-courses"));
                    window.__vShortcutActive = true;
                    return;
                case "g":
                    event.preventDefault();
                    btn = navBtn;
                    if (btn?.getAttribute("aria-expanded") === "false") simulateClickWhenReady(btn);
                    typedInput = "planner";
        ...
        if (event.key === "Enter") {
            ...
            if (window.__vShortcutActive) {
                try {
                    ...
                    if (firstLi?.href) {
                        localStorage.setItem("openDocuments", "true");
        ...
        if (event.key === "Escape") {
            if (window.__gEventActive && navBtn) {
                simulateClickWhenReady(navBtn); // close g menu
                window.__gEventActive = false;
            }
            if (window.__lEventActive && linksBtn) {
                simulateClickWhenReady(linksBtn); // close l menu
                window.__lEventActive = false;
            }
            typedInput = "";
            selectedIndex = -1;
